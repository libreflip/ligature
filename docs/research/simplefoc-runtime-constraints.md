...
Hardware-model correction: 2026-08-01. The checked-in MKS ESP32 FOC V1.0
schematic marks GPIO 21/22 unconnected and shows no separate driver-enable
net. In this report, a software-disabled driver means zero/low PWM commands;
only removal of motor power provides physical isolation.
...
| Keep every phase PWM command off for the whole startup | `BLDCMotor::init()` briefly sets the motor's software state enabled, but a three-pin `BLDCDriver3PWM` writes zero PWM and has no hardware enable to assert | Establish all six phase-control latches low before peripheral initialization, explicitly zero feed-forward/targets, and verify boot/reset PWM behavior on hardware |
...
Do not persist ADC zero offsets. [`InlineCurrentSense::init()`](https://github.com/simplefoc/Arduino-FOC/blob/v2.4.0/src/current_sense/InlineCurrentSense.cpp#L32-L87) averages 1000 zero-current readings to calculate per-pin offsets, so they should be recalculated at every boot with all PWM commands off and no phase current. Restore pin order and gain signs before calling `init()` so each newly measured offset remains associated with the correct pin.

A safe application-owned startup sequence is:

1. Set the output latches for all six phase-control GPIOs low before configuring them as outputs or initializing PWM peripherals.
2. Initialize the Hall inputs and interrupts, then validate the raw state and the versioned commissioning record.
3. Initialize the driver and immediately command zero PWM.
4. Configure the motor, link its driver, explicitly set both feed-forward vectors to `{0, 0}`, call `motor.init()`, and immediately disable it in software. Confirm that the driver duty fields remain zero.
5. Configure the restored current-sense pin order and gain signs, initialize it with PWM off, and link it to the motor.
...
7. Remain disarmed. On an ordinary arm request, initialize angle target to the current shaft angle and current target to zero before enabling active control.

[`BLDCMotor::init()`](https://github.com/simplefoc/Arduino-FOC/blob/v2.4.0/src/BLDCMotor.cpp#L65-L118) waits 500 ms, calls `enable()`, and waits another 500 ms. With this board's three-pin [`BLDCDriver3PWM`](https://github.com/simplefoc/Arduino-FOC/blob/v2.4.0/src/drivers/BLDCDriver3PWM.cpp), there is no enable pin to assert; `enable()` changes software state and writes zero PWM. Immediate `motor.disable()` returns to an explicitly inactive state, but neither action isolates bridge power. Boot/reset tests still need to verify that the phase-control GPIOs never produce an unintended pulse while ownership moves from GPIO latches to PWM peripherals.
...
- **Commission**: allowed only under explicit service conditions, with conservative alignment voltage/current, hardware power removal available, an untrusted-position state, and no ordinary motion commands. Begin from `Direction::UNKNOWN`, unset electrical zero, baseline current pin mapping/gains, and `skip_align = false`. Persist the final fields only after `initFOC()` returns success and post-checks pass, then command zero PWM and mark the motor inactive immediately.
...
A serial stop command cannot preempt a control task blocked inside unmodified `initFOC()`, and this board has no hardware enable that another task can deassert. The production solution therefore needs a cooperative alignment state machine (local patch or project-owned equivalent) that advances one bounded step per control iteration, checks the stop latch between steps, commands zero PWM on abort, discards calibration results, and latches a fault. It needs a physical test proving stop latency during every alignment phase. During the simple smoke test's accepted blocking alignment, pulling mains is the only independent stop; closing serial cannot force PWM off. Software must not replace that external power-cut method during first commissioning.
...
- **Required:** replace blocking production alignment with cooperative/cancellable commissioning so a stop can promptly command zero PWM; there is no hardware-enable fallback.
...
- **Decision:** verify the no-pulse transition from low GPIO latches to PWM ownership during `BLDCMotor::init()`; do not model the motor software-enabled flag as bridge-power isolation.
...
1. **Board identity:** populated current amplifier part, shunt value, bridge channel, current pins, phase-to-pin mapping, absence of a hardware-enable net, and Hall-to-machine direction mapping.
...
3. **Commissioning safety:** safe alignment voltage/current, complete result record, record mismatch/corruption rejection, power loss during every phase, no unintended boot/reset PWM pulse, and stop-to-zero-PWM latency during every alignment phase.
